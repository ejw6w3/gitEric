...
int main()
{
	...
	if(obj1 > obj2)	// normal expression. Which operator is being called?	> is being called
	...
	if(obj2 > obj1)	// Which operator is being called?	> is being called
	...
	obj2*f;		// Would f*obj2 work? Give it a try.
				//These would not work because f and 2 are not vectors
	obj1.Printvals();
	obj2.Printvals();
	...
	obj2.Printvals();					//You could make it work by defining an = operator
	obj3.Printvals();
	
	obj3 = operator+(obj2, obj1);	// is this the same as before?
						//Yes does the same as the call above
	obj1.Printvals();
	obj2.Printvals();
	obj3.Printvals();
